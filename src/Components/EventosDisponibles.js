              ...
              <Col key={evento.id} lg={4} md={6} className="mb-4">
                <Card className="evento-card h-100 border-success">
                  ...
                  <Card.Body className="d-flex flex-column">
                    <Card.Title className="text-success">{evento.nombre}</Card.Title>
                    <Card.Text className="flex-grow-1">{evento.descripcion}</Card.Text>
                    <div className="evento-detalles mt-3">
                      <p className="evento-fecha mb-1">
                        <FaCalendarAlt className="text-success me-2" /> {evento.fecha}
                      </p>
                      <p className="evento-categoria mb-1">
                        <FaCode className="text-success me-2" /> {evento.categoria}
                      </p>
                      <p className="contador-suscripciones mb-1">
                        <FaUsers className="text-success me-2" /> 
                        <strong>{contadorEventos[evento.id] || 0}</strong> suscripciones
                      </p>
                    ...
                    <div className="button-container mt-3">
                      <Button
                        variant={isSuscrito(evento.id) ? "outline-danger" : "success"}
                        onClick={() => handleToggleSuscripcion(evento)}
                        className={`btn-suscribir w-100 ${isSuscrito(evento.id) ? 'btn-cancelar' : ''} ${animatingEventId === evento.id ? 'animating' : ''}`}
                        disabled={!isSuscrito(evento.id) && evento.cuposDisponibles <= 0}
                      >
                    ...
                    >
                      {evento.cuposDisponibles > 0 ? `${evento.cuposDisponibles} cupos` : "Agotado"}
                    </Badge>
                  </div>
                </Card.Body>
              </Card>
